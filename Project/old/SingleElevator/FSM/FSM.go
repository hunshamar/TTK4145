...
func StateMachine(elevatorNumber int, port string){
	...
	timer.WatchdogInit(int64(3000),&watchdogTimer)
		...
		select{
			case b := <-buttonPress:
				...
				case dataTypes.BT_HallDown:
					elevator.LocalOrders[b.Button][b.Floor] = dataTypes.O_Received // To two later
				}

				if elevatorLogic.ShouldStopHere(elevator) && elevator.State != dataTypes.S_Moving{ // Bestilling på etasje
